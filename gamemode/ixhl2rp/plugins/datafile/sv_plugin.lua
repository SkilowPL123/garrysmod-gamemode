...
function PLUGIN:ReturnPermission(CID, RegID, callback)
	if not CID or not RegID then
		error("CID or RegID is nil")
		return
	end

	local query = mysql:Select("ix_items")
		query:Select("data")
		query:WhereLike("data", "\"cid\":\""..tostring(CID).."\"")
		query:WhereLike("data", "\"number\":\""..tostring(RegID).."\"")
...
function PLUGIN:SetRankStatus(datafileID, rank, client, char)
	if not ranks[rank] then return end
...
function PLUGIN:AddEntry(poster, player, category, text, points)
	...
	if not posterCharacter then return end
	...
	for _, v in ipairs(rf:GetPlayers()) do
		if v.lastDatafile ~= id or not v:GetCharacter() then
			rf:RemovePlayer(v)
		else
			continue
		end
...
function PLUGIN:SetBOL(poster, player, bBOL, reason)
	...
	elseif bBOL == false then
		...
		genericdata.bol = not genericdata.bol
