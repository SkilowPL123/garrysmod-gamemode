...
ix.lang.AddTable("english", {
	optHeadBobScale = "Head bob scale",
	optdHeadBobScale = "How much intensive the head bobs.",
})
ix.lang.AddTable("russian", {
	optHeadBobScale = "Skala machania głową",
	optdHeadBobScale = "Jak bardzo intensywne machanie głową.",
})
...
if (CLIENT) then
	...
	ix.option.Add("headBobScale", ix.type.number, 1, {
	...
	local function CanDoHeadBob(client)
		if (hookRun("ShouldHeadbob", client) == false or
			...
			ixOptionGet("headBobScale", 1) == 0 or
	...
	function PLUGIN:PlayerAdjustHeadBobInfo(client, info)
		local scale = ix.option.Get("headBobScale", 1)
