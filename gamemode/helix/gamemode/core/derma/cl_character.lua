...
function PANEL:Init()
	...
	self.addpaint.Paint = function(this, w, h)
		...
		local text = "Wersja beta: Niektóre funkcje mogą być niedostępne lub mogą nie działać zgodnie z oczekiwaniami."
	...
	charcreate:SetText('NOWA POSTAĆ')
	...
	self.loadbutton:SetText('POSTACIE')
	self.loadbutton:SetIcon(2)
	self.loadbutton:SizeTo(285, 40, 1, (0.25 * 2))
	self.loadbutton.DoClick = function()
		if (!bHasCharacter) then
			--self.loadButton:SetDisabled(true)
			self:GetParent():ShowNotice(3, "Musisz mieć co najmniej jedną postać!")
		else
			self:Dim()
			parent.loadCharacterPanel:SlideUp()
		end
	...
	content:SetText('PACZKA')
	content:SetIcon(4)
	content:SizeTo(285, 40, 1, (0.25 * 4))
	content.DoClick = function()
		gui.OpenURL("https://steamcommunity.com/sharedfiles/filedetails/?id=3413038153")
	end
	...
	a:SetText(self.bUsingCharacter and "Z POWROTEM" or "WYJŚCIE")
	a:SetIcon(5)
